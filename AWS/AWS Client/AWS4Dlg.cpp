...
bool CAWS4Dlg::GetFileProperties(const CString pszProcessPath, CString& resStr)
{
    ...
    struct LANGANDCODEPAGE { // структура дл€ получени€ кодовых страниц по €зыкам трансл€ции ресурсов файла
        WORD wLanguage;
        WORD wCodePage;
    } *pLangCodePage;
 
    // имена параметров, значени€ которых будем запрашивать
...
//            _T("Author")
    ...
    TCHAR paramNameBuf[256]; // здесь формируем им€ параметра
    TCHAR *paramValue;       // здесь будет указатель на значение параметра, который нам вернет функци€ VerQueryValue
    UINT paramSz;            // здесь будет длина значени€ параметра, который нам вернет функци€ VerQueryValue
    ...
    if ( infoSize > 0 )
    {
        ...
        {
            // информаци€ находитс€ блоками в виде "\StringFileInfo\кодова€_страница\им€_параметра
            // т.к. мы не знаем заранее сколько и какие локализации (кодова€_страница) ресурсов имеютс€ в файле,
            // то будем перебирать их все
 
            UINT cpSz;
            // получаем список кодовых страниц
            if ( VerQueryValue(infoBuffer,                      // наш буфер, содержащий информацию
                               _T("\\VarFileInfo\\Translation"),// запрашиваем имеющиес€ трансл€ции
                               (LPVOID*) &pLangCodePage,        // сюда функци€ вернет нам указатель на начало интересующих нас данных
                               &cpSz) )                         // а сюда - размер этих данных
            {
                ...
                for (int cpIdx = 0; cpIdx < (int)(cpSz/sizeof(struct LANGANDCODEPAGE)); cpIdx++ )
                {
                    ...
                    {
                        // формируем им€ параметра ( \StringFileInfo\кодова€_страница\им€_параметра )
                        _stprintf(paramNameBuf, _T("\\StringFileInfo\\%04x%04x\\%s"),
                                        pLangCodePage[cpIdx].wLanguage,  // ну, или можно сделать фильтр дл€
